...
struct CreateUser: Snippet
{
    ...
    func execute(with completion: @escaping (_ result: Result) -> Void)
    {
        ...
        Snippets.graphClient.users().request()?.add(newUser, withCompletion: { (createdUser, error) in
...
struct GetMembers: Snippet
{
    ...
    func execute(with completion: @escaping (_ result: Result) -> Void)
    {
        ...
        Snippets.graphClient.groups(groupId).members().request().getWithCompletion({ (memberCollection, nextRequest, error) in
            if let nsError = error as NSError? {
                completion(.Failure(error: MSGraphError.NSErrorType(error: nsError)))
                return
            } else {
...
struct GetOwners: Snippet
{
    ...
    func execute(with completion: @escaping (_ result: Result) -> Void)
    {
        ...
        Snippets.graphClient.groups(groupId).owners().request().getWithCompletion({ (memberCollection, nextRequest, error) in
            if let nsError = error as NSError? {
                completion(.Failure(error: MSGraphError.NSErrorType(error: nsError)))
                return
            } else {
...
struct CreateGroup: Snippet
{
    ...
    func execute(with completion: @escaping (_ result: Result) -> Void)
    {
        ...
        Snippets.graphClient.groups().request().add(group) { (addedGroup, error) in
            if let nsError = error as NSError? {
                completion(.Failure(error: MSGraphError.NSErrorType(error: nsError)))
                return
            } else {
                ...
                let displayName = addedGroup?.displayName ?? ""
                completion(.Success(displayText: "Group \(displayName) was added"))
            }
        }
    }
}
    ...
    func execute(with completion: @escaping (_ result: Result) -> Void) {
        ...
        Snippets.graphClient.groups(groupId).request().getWithCompletion { (group, error) in
            if let nsError = error as NSError? {
                ...
                Snippets.graphClient.groups(validGroup.entityId).request().update(validGroup, withCompletion: { (group, error) in
                    if let nsError = error as NSError? {
                        completion(.Failure(error: MSGraphError.NSErrorType(error: nsError)))
                        return
                    } else {
                        let displayName = group?.displayName ?? ""
                        completion(.Success(displayText: "Group \(displayName) updated"))
                    }
                })
            }
        }
